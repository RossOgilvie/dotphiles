...
if which tmux &> /dev/null
	...
	local zsh_tmux_ross_plugin_path="$(cd "$(dirname "$0")" && pwd)"
	...
	then
		...
		export _ZSH_TMUX_FIXED_CONFIG="$zsh_tmux_ross_plugin_path/tmux-ross.extra.conf"
	else
	...
	function _zsh_tmux_plugin_run()
	{
		...
		then
			...
			tmux_nb=$(tmux ls | grep "^$zsh_tmux_ross_session_name" | wc -l)

			if [[ "$tmux_nb" == "0" ]]; then
			    echo "Launching tmux base session $zsh_tmux_ross_session_name ..."
			    \tmux `[[ "$ZSH_TMUX_ITERM2" == "true" ]] && echo '-CC'` `[[ "$ZSH_TMUX_FIXTERM" == "true" ]] && echo '-f '$_ZSH_TMUX_FIXED_CONFIG` new-session -s $zsh_tmux_ross_session_name
			else
			    ...
			    if [[ -z "$TMUX" ]]; then
			        ...
			        # to share windows
			        \tmux `[[ "$ZSH_TMUX_ITERM2" == "true" ]] && echo '-CC'` `[[ "$ZSH_TMUX_FIXTERM" == "true" ]] && echo '-f '$_ZSH_TMUX_FIXED_CONFIG` new-session -d -t $zsh_tmux_ross_session_name -s $session_id
